...
use super::packet::{self, RCODE_FORMERR};
use super::resolver;

/// Run a DNS UDP server on the given address.
pub async fn run_udp_server(addr: SocketAddr, state: SharedState) -> Result<()> {
...
pub async fn run_tcp_server(addr: SocketAddr, state: SharedState) -> Result<()> {
...
async fn handle_dns_query(query_bytes: &[u8], state: &SharedState, src: SocketAddr) -> Vec<u8> {
    // Parse query
