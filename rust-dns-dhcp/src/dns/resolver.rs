...
use super::packet::{self, DnsQuery, RCODE_NOERROR, RCODE_NXDOMAIN, RCODE_SERVFAIL};
...
/// 5. Cache
/// 6. Upstream forward
pub async fn resolve(query: &DnsQuery, state: &SharedState) -> ResolveResult {
    ...
    if config.dns.expand_hosts && !config.dns.local_domain.is_empty() {
        let hostname = if let Some(stripped) = name.strip_suffix(&format!(".{}", config.dns.local_domain)) {
        ...
        if let Some(hostname) = hostname {
            if let Some(ip) = state_read.lease_store.find_ip_by_hostname(&hostname) {
    ...
    // 2. Static records
    for static_rec in &config.dns.static_records {
    ...
    if !config.dns.local_domain.is_empty() {
        let is_local = name.ends_with(&format!(".{}", config.dns.local_domain))
            || *name == config.dns.local_domain;

        if is_local {
            let records = match qtype {
                RecordType::A if !config.dns.wildcard_ipv4.is_empty() => {
                    if let Ok(ip) = config.dns.wildcard_ipv4.parse::<Ipv4Addr>() {
                        vec![DnsRecord::a(name, ip, 300)]
                    } else {
                        vec![]
                    }
                }
                RecordType::AAAA if !config.dns.wildcard_ipv6.is_empty() => {
                    if let Ok(ip) = config.dns.wildcard_ipv6.parse::<Ipv6Addr>() {
    ...
    // 4. Adblock filter
    if config.adblock.enabled && state_read.adblock.is_blocked(name) {
        debug!("Blocked {} via adblock", name);
        let records = match config.adblock.block_response.as_str() {
            "zero_ip" => match qtype {
...
fn parse_static_record(name: &str, rec: &crate::config::StaticRecord, rtype: RecordType) -> Option<DnsRecord> {
    match rtype {
        RecordType::A => {
