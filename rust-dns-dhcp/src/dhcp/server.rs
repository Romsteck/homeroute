...
pub async fn run_dhcp_server(state: SharedState) -> Result<()> {
    ...
    #[cfg(target_os = "linux")]
    if !config.dhcp.interface.is_empty() {
        socket.bind_device(Some(config.dhcp.interface.as_bytes()))?;
        info!("DHCP bound to interface {}", config.dhcp.interface);
    }
    ...
    loop {
        ...
        let config = state_write.config.dhcp.clone();
