...
function GroupBadge({ group, color, label, showIcon = true, size = 'sm' }) {
  ...
  if (builtIn) {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 rounded-full border ${builtIn.bg} ${builtIn.border} ${builtIn.text} ${sizeClasses[size]}`}
      >
  ...
  // Custom group with dynamic color
