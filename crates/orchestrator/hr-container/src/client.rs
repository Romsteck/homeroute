...
impl NspawnClient {
    ...
    async fn wait_ready(name: &str) -> Result<()> {
        for i in 0..30 {
            ...
            if i == 29 {
                warn!(
                    container = name,
                    "Container network not ready after 30s, proceeding anyway"
                );
            }
        }
        Ok(())
    ...
    pub async fn wait_for_network(container: &str, timeout_secs: u32) -> Result<()> {
        ...
        warn!(
            container,
            timeout_secs, "Network connectivity not confirmed after timeout, proceeding anyway"
        );
        Ok(())
    }
    ...
    pub async fn start_container(name: &str) -> Result<()> {
        ...
        // Re-enable the systemd unit so it starts on boot and systemd
        // won't consider it masked/disabled.
        ...
        if !enable_out.status.success() {
            ...
            warn!(
                container = name,
                "systemctl enable failed (non-fatal): {stderr}"
            );
        }
    ...
    pub async fn stop_container(name: &str) -> Result<()> {
        ...
        // Disable the systemd unit first so systemd won't auto-restart
        // the container (machines.target.wants symlink is removed).
        let service = format!("systemd-nspawn@{name}.service");
        let disable_out = Command::new("systemctl")
            .args(["disable", &service])
            .output()
            .await
            .context("failed to run systemctl disable")?;
        if !disable_out.status.success() {
            let stderr = String::from_utf8_lossy(&disable_out.stderr);
            warn!(
                container = name,
                "systemctl disable failed (non-fatal): {stderr}"
            );
        }
    ...
    /// `network_mode` is "bridge:<bridge_name>" (e.g. "bridge:br0").
    ...
    pub async fn write_nspawn_unit(
        ...
        tokio::fs::create_dir_all(NSPAWN_UNIT_DIR)
            .await
            .context("failed to create nspawn unit directory")?;

        let network_line = if let Some(bridge) = network_mode.strip_prefix("bridge:") {
        ...
        let directory_line = if storage_path != Path::new(DEFAULT_STORAGE) {
            ...
            tokio::fs::symlink(&rootfs, &symlink_path)
                .await
                .with_context(|| {
                    format!(
    ...
    pub async fn write_network_config(
        name: &str,
        storage_path: &Path,
        static_ip: Option<Ipv4Addr>,
    ) -> Result<()> {
        ...
        tokio::fs::write(
            network_dir.join("80-container-host0.network"),
            &network_config,
        )
        .await
