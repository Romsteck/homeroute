...
impl AgentRegistry {
    ...
    /// Explicitly set enabled/disabled for an application. Used by container
    /// start/stop so the health watcher respects manual state changes.
    pub async fn set_application_enabled(&self, id: &str, enabled: bool) -> Result<()> {
        let mut state = self.state.write().await;
    ...
    /// (e.g. after a host recovery), restore its Connected status.
    ...
    /// (e.g. after a host recovery cycle).
    ...
    pub async fn on_host_connected(
        ...
        let mut conns = self.host_connections.write().await;
        if let Some(existing) = conns.get(&host_id) {
            warn!(
                host_id = %host_id,
                existing_host = %existing.host_name,
                new_host = %host_name,
                "Replacing existing host-agent connection (duplicate host_name?)"
            );
        }
        conns.insert(host_id.clone(), conn);
        drop(conns);
    ...
    pub async fn request_wake_host(&self, host_id: &str) -> Result<WakeResult, String> {
        let (current_state, cached_mac) = {
            let states = self.host_power_states.read().await;
            match states.get(host_id) {
                Some(info) => (info.state, info.mac_address.clone()),
                None => (HostPowerState::Offline, None),
            }
        };
        ...
        match current_state {
            ...
            HostPowerState::WakingUp => {
                ...
                let states = self.host_power_states.read().await;
                if let Some(info) = states.get(host_id) {
                    if (Utc::now() - info.since).num_seconds() < 30 {
