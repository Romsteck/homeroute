//! WebSocket connection to hr-orchestrator.
//!
//! Handles:
//! - Auth handshake (EnvAgentMessage::Auth)
//! - Heartbeat loop (every 30s with apps_running/apps_total)
//! - Receiving commands (EnvOrchestratorMessage)
//! - Sending events (EnvAgentMessage)
//! - IP change detection
...
use hr_environment::protocol::{EnvAgentMessage, EnvOrchestratorMessage};
use hr_environment::EnvAgentConfig;

/// Connect to the orchestrator, authenticate, and run the bidirectional message loop.
///
/// - `orchestrator_tx`: forwards received `EnvOrchestratorMessage`s to the main loop.
/// - `outbound_rx`: receives `EnvAgentMessage`s to send to the orchestrator.
pub async fn run_connection(
    config: &EnvAgentConfig,
    orchestrator_tx: mpsc::Sender<EnvOrchestratorMessage>,
    mut outbound_rx: mpsc::Receiver<EnvAgentMessage>,
) -> Result<()> {
    let url = config.orchestrator_ws_url();
    info!(url, "Connecting to HomeRoute orchestrator");
    ...
    // Track initial IP for change detection
    let initial_ip = ipv4_address.clone();

    // Count apps for heartbeat
    let apps_total = config.apps.len() as u32;

    // Send Auth message
    let auth_msg = EnvAgentMessage::Auth {
        token: config.token.clone(),
        env_slug: config.env_slug.clone(),
    ...
    let auth_result: EnvOrchestratorMessage = match first_msg {
        Message::Text(text) => serde_json::from_str(&text)?,
        other => anyhow::bail!("Unexpected message type during auth: {other:?}"),
    };

    match auth_result {
        EnvOrchestratorMessage::AuthResult { success: true, .. } => {
            info!("Authentication successful");
        }
        EnvOrchestratorMessage::AuthResult {
            success: false,
            error,
            ..
        } => {
        ...
        loop {
            ...
            if let Some(new_ip) = detect_ipv4_address(&ip_check_interface).await {
                if prev_ip.as_ref() != Some(&new_ip) {
                    info!(new_ip, prev = ?prev_ip, "IPv4 address changed, notifying orchestrator");
    ...
    loop {
        tokio::select! {
            ...
            ws_msg = ws_stream.next() => {
                match ws_msg {
                    Some(Ok(Message::Text(text))) => {
                        match serde_json::from_str::<EnvOrchestratorMessage>(&text) {
                            Ok(msg) => {
                                let is_shutdown = matches!(&msg, EnvOrchestratorMessage::Shutdown);
                                if orchestrator_tx.send(msg).await.is_err() {
                                    error!("Orchestrator message channel closed");
                                    break;
                                }
                                if is_shutdown {
                            ...
                            Err(e) => {
                                warn!("Invalid message from orchestrator: {e}");
                            }
                        }
                    }
            ...
            // Outbound messages (app discovery, metrics, pipeline progress, etc.)
            ...
            Some(new_ip) = ip_check_rx.recv() => {
                info!(new_ip, "Sending re-auth with updated IP");
...
pub async fn ensure_network_up(interface: &str) {
    ...
    for line in stdout.lines() {
        ...
        let Some(iface_raw) = parts.get(1) else {
            continue;
        };
        let iface = iface_raw
            .trim_end_matches(':')
        ...
        if let Some(addr_idx) = parts.iter().position(|&p| p == "inet") {
            if let Some(addr_cidr) = parts.get(addr_idx + 1) {
                ...
                info!(
                    ...
                    "IPv4 detected via fallback (interface mismatch)"
                );
