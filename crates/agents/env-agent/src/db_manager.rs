//! Database manager — wraps hr-db DataverseEngine per app.
//!
//! Each app gets its own SQLite database at {db_path}/{slug}.db.
//! The DbManager opens engines on demand and keeps them cached.
...
/// Manages per-app Dataverse databases.
...
#[allow(dead_code)]
impl DbManager {
    /// Create a new DbManager with the given base path.
    pub fn new(db_path: &Path) -> Self {
        std::fs::create_dir_all(db_path).ok();
        info!(path = %db_path.display(), "DbManager initialized");
        Self {
    ...
    pub fn db_path_for(&self, app_slug: &str) -> PathBuf {
        self.db_path.join(format!("{}.db", app_slug))
    }
    ...
    pub async fn get_engine(&self, app_slug: &str) -> Result<Arc<Mutex<DataverseEngine>>, String> {
        // Fast path: already cached
        {
            let engines = self.engines.read().await;
            if let Some(engine) = engines.get(app_slug) {
                return Ok(engine.clone());
            }
        }

        // Slow path: open and cache
        let db_path = self.db_path_for(app_slug);
        let engine = DataverseEngine::open(&db_path)
            .map_err(|e| format!("Failed to open database for '{}': {}", app_slug, e))?;
        let engine = Arc::new(Mutex::new(engine));

        let mut engines = self.engines.write().await;
        engines.insert(app_slug.to_string(), engine.clone());
        info!(app_slug, path = %db_path.display(), "Database opened");
        Ok(engine)
    }
    ...
    pub async fn execute_query(
        ...
        let engine = self.get_engine(app_slug).await?;
    ...
    pub async fn get_schema(&self, app_slug: &str) -> Result<Value, String> {
        let db_path = self.db_path_for(app_slug);
        if !db_path.exists() {
            return Err(format!("No database for app '{}'", app_slug));
        }
    ...
    pub async fn overview(&self) -> Result<Value, String> {
        ...
        let entries = std::fs::read_dir(&self.db_path).map_err(|e| e.to_string())?;
    ...
    /// List all app slugs that have databases.
    ...
    /// Create a snapshot backup of an app's database.
    ///
    /// Copies `{app_slug}.db` to `{app_slug}.db.bak.{timestamp}`.
    /// Returns the path to the backup file.
    pub async fn snapshot_db(&self, app_slug: &str) -> Result<PathBuf, String> {
        let db_path = self.db_path_for(app_slug);
        if !db_path.exists() {
            return Err(format!("No database for app '{}'", app_slug));
        }
        ...
        let backup_path = self
            .db_path
            .join(format!("{}.db.bak.{}", app_slug, timestamp));
