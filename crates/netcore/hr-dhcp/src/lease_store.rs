...
/// Duration (seconds) for which a declined IP is kept out of the pool.
const CONFLICT_HOLD_SECS: u64 = 600; // 10 minutes
...
pub struct LeaseStore {
    ...
    /// IPs marked as conflicted by DHCPDECLINE — value is the unix timestamp
    /// at which the hold expires and the IP can be re-offered.
    conflicted_ips: HashMap<Ipv4Addr, u64>,
    file_path: PathBuf,
}
...
impl LeaseStore {
    ...
    /// Mark an IP as conflicted (DHCPDECLINE). The IP will not be offered
    /// for [`CONFLICT_HOLD_SECS`] seconds.
    pub fn mark_conflicted(&mut self, ip: Ipv4Addr) {
        let hold_until = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_secs()
            + CONFLICT_HOLD_SECS;
        self.conflicted_ips.insert(ip, hold_until);
    }

    /// Returns `true` if the IP is currently in the conflict-hold period.
    pub fn is_conflicted(&self, ip: Ipv4Addr) -> bool {
        if let Some(&hold_until) = self.conflicted_ips.get(&ip) {
            let now = SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
                .as_secs();
    ...
    /// Purge expired leases and expired conflict holds
    ...
    pub fn allocate_ip(
        ...
        for ip_int in start..=end {
            ...
            if !self.is_ip_in_use(ip) && !self.is_conflicted(ip) {
