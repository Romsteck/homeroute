...
async fn list_hosts(State(state): State<ApiState>) -> Json<Value> {
    ...
    if let Some(arr) = hosts.as_array_mut() {
        for host in arr.iter_mut() {
            if let Some(id) = host.get("id").and_then(|i| i.as_str()).map(|s| s.to_string()) {
                ...
                // Include power state from connection data
                if let Some(conn) = conn_data {
    ...
    let local_host = {
        let (lan_interface, container_storage_path) = match state.orchestrator.request(&OrchestratorRequest::GetContainerConfig).await {
            Ok(r) if r.ok => {
                let d = r.data.unwrap_or(json!({}));
                let li = d.get("lan_interface").and_then(|v| v.as_str()).map(|s| s.to_string());
                let sp = d.get("container_storage_path").and_then(|v| v.as_str()).unwrap_or("/var/lib/machines").to_string();
                (li, sp)
            }
            _ => (None, "/var/lib/machines".to_string()),
        };
...
async fn wake(Path(id): Path<String>, State(state): State<ApiState>) -> Json<Value> {
    match state.orchestrator.request(&OrchestratorRequest::WakeHost {
        ...
        Ok(r) => {
            let err = r.error.unwrap_or_else(|| "Failed to wake host".to_string());
            Json(json!({"success": false, "error": err}))
        }
        Err(e) => {
            // Fallback: direct WOL if orchestrator unavailable
            let data = load_hosts().await;
            let host = match find_host(&data, &id) {
                Some(h) => h,
...
async fn bulk_wake(State(state): State<ApiState>, Json(body): Json<BulkRequest>) -> Json<Value> {
    ...
    for id in &body.host_ids {
        match state.orchestrator.request(&OrchestratorRequest::WakeHost {
            ...
            Ok(r) => {
                let err = r.error.unwrap_or_else(|| "Failed".to_string());
                results.push(json!({"id": id, "success": false, "error": err}));
            }
            Err(_) => {
                // Fallback: direct WOL
                let data = load_hosts().await;
...
async fn set_auto_off(
    ...
    // Push to connected agent via IPC
    let cmd = json!({
        "type": "set_auto_off",
        "mode": body.mode,
...
async fn get_host_metrics(Path(id): Path<String>, State(state): State<ApiState>) -> Json<Value> {
    ...
    if let Some(conn) = connections.get(&id) {
        if let Some(metrics) = conn.get("metrics") {
            return Json(json!({"success": true, "metrics": metrics}));
        }
    }
...
async fn exec_on_host(
    ...
    match state.orchestrator.request_long(&OrchestratorRequest::ExecRemoteContainer {
        ...
        Ok(r) if r.ok => {
            let data = r.data.unwrap_or(json!({}));
            Json(json!({
                "success": data.get("success").and_then(|s| s.as_bool()).unwrap_or(true),
                "stdout": data.get("stdout").and_then(|s| s.as_str()).unwrap_or(""),
                        ...
                        if let Ok(agent_msg) = serde_json::from_str::<HostAgentMessage>(&text) {
                            match agent_msg {
                                ...
                                HostAgentMessage::BackupRepoReady { transfer_id } => {
                                    tracing::warn!(transfer_id = %transfer_id, "Legacy BackupRepoReady (ignored, backup runs locally)");
                                }
                                HostAgentMessage::BackupRepoComplete { transfer_id, repo_name, success: _, message: _, snapshot_name: _ } => {
                                    tracing::warn!(transfer_id = %transfer_id, repo = %repo_name, "Legacy BackupRepoComplete (ignored, backup runs locally)");
                                }
