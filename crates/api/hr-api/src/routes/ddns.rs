...
/// Query hr-edge for cloud relay enabled status.
async fn is_relay_enabled(state: &ApiState) -> bool {
    state.edge.request(&EdgeRequest::CloudRelayStatus).await
        .ok()
        .and_then(|r| r.data)
...
async fn status(State(state): State<ApiState>) -> Json<Value> {
    ...
    let relay_enabled = is_relay_enabled(&state).await;
