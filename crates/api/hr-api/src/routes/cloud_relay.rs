...
async fn get_status(State(state): State<ApiState>) -> Json<RelayStatusResponse> {
    ...
    // Query hr-edge for cloud relay status via IPC
    let edge_status = state.edge.request(&EdgeRequest::CloudRelayStatus).await.ok();
    let edge_data = edge_status.as_ref().and_then(|r| r.data.as_ref());

    let enabled = edge_data
        .and_then(|d| d.get("enabled"))
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    let status_str = edge_data
        .and_then(|d| d.get("status"))
        .and_then(|v| v.as_str())
        .unwrap_or("disconnected")
        .to_string();
    let latency_ms = edge_data
        .and_then(|d| d.get("latency_ms"))
        .and_then(|v| v.as_u64());
    let active_streams = edge_data
        .and_then(|d| d.get("active_streams"))
        .and_then(|v| v.as_u64())
        .map(|v| v as u32);
    ...
    let vps_ipv4 = edge_data
        .and_then(|d| d.get("vps_ipv4"))
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
...
async fn enable_relay(
    ...
    if let (Some(token), Some(zone_id)) = (&state.env.cf_api_token, &state.env.cf_zone_id) {
        ...
        let app_slugs: Vec<String> = match state.orchestrator.request(&OrchestratorRequest::ListApplications).await {
            Ok(r) if r.ok => {
                r.data.and_then(|d| d.as_array().map(|arr| {
                    arr.iter().filter_map(|a| a.get("slug").and_then(|s| s.as_str()).map(String::from)).collect()
                })).unwrap_or_default()
            }
            _ => vec![],
        };
    ...
    // Tell hr-edge to enable cloud relay
    let resp = state.edge.request(&EdgeRequest::CloudRelayEnable).await
...
async fn disable_relay(
    ...
    // Tell hr-edge to disable cloud relay
    let resp = state.edge.request(&EdgeRequest::CloudRelayDisable).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    if !resp.ok {
        return Err((StatusCode::INTERNAL_SERVER_ERROR, resp.error.unwrap_or_default()));
    }
...
/// POST /api/cloud-relay/update -- Push the local hr-cloud-relay binary to VPS via edge IPC.
async fn push_update(
    ...
    // 3. Write binary to a temp file for hr-edge to read
    let tmp_path = format!("/tmp/hr-cloud-relay-update-{}.bin", std::process::id());
    tokio::fs::write(&tmp_path, &binary_data)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to write temp file: {}", e)))?;

    // 4. Send push command via edge IPC
    let resp = state.edge.request_with_timeout(
        &EdgeRequest::CloudRelayPushBinary {
            path: tmp_path.clone(),
            sha256,
        },
        ...
        .map_err(|e| {
            ...
            (StatusCode::SERVICE_UNAVAILABLE, format!("Edge IPC error: {}", e))
        })?;
    ...
    let message = resp.data
        ...
        .unwrap_or_else(|| "Binary pushed to VPS".to_string());
