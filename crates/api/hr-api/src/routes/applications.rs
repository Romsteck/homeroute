...
/// Resolve a dev app to its linked prod via orchestrator IPC.
/// Returns (prod_id, container_name, host_id) or an error response.
async fn resolve_linked_prod_ipc(
    state: &ApiState,
    dev_id: &str,
) -> Result<(String, String, String), axum::response::Response> {
    match state
        .orchestrator
        .request(&OrchestratorRequest::ResolveLinkedProd {
        ...
        .await
    {
        Ok(resp) if resp.ok => {
            ...
            let prod_id = d["prod_id"].as_str().unwrap_or("").to_string();
            let container_name = d["container_name"].as_str().unwrap_or("").to_string();
            let host_id = d["host_id"].as_str().unwrap_or("").to_string();
            if prod_id.is_empty() {
                Err((
                    StatusCode::BAD_REQUEST,
                    Json(serde_json::json!({"success": false, "error": "No linked production application"})),
                )
                    .into_response())
            } else {
                Ok((prod_id, container_name, host_id))
            }
        }
        Ok(resp) => Err((
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({"success": false, "error": resp.error.unwrap_or_else(|| "Resolution failed".into())})),
        )
            .into_response()),
        Err(e) => Err(ipc_err_response(e)),
    }
}
...
/// Writes binary to temp file, then delegates to hr-orchestrator via IPC.
async fn deploy_to_production(
    ...
    if let Err(e) = tokio::fs::write(&tmp_path, &body).await {
        return (
            ...
            Json(serde_json::json!({"success": false, "error": format!("Failed to write temp binary: {e}")})),
        )
            .into_response();
    }

    let binary_size = body.len();
    info!(dev_id, binary_bytes = binary_size, "Deploy binary to production via IPC");

    match state
        .orchestrator
        .request_long(&OrchestratorRequest::DeployToProduction {
            dev_id: dev_id.clone(),
            binary_path: tmp_path.clone(),
        })
        .await
    {
        Ok(resp) if resp.ok => {
            ...
            let prod_id = resp
                .data
                .as_ref()
                .and_then(|d| d.get("prod_id"))
                .and_then(|v| v.as_str())
                .unwrap_or("");
            Json(serde_json::json!({
                "success": true,
                "message": resp.data.as_ref().and_then(|d| d.get("message")).and_then(|v| v.as_str()).unwrap_or("Deploy completed"),
                "prod_id": prod_id,
        ...
        Ok(resp) => {
            let _ = tokio::fs::remove_file(&tmp_path).await;
            error!(dev_id, "Deploy failed: {:?}", resp.error);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
async fn get_prod_status(
    ...
    let (_, prod_container, prod_host) = match resolve_linked_prod_ipc(&state, &dev_id).await {
        Ok(v) => v,
        Err(e) => return e,
    };
    ...
    match state
        .orchestrator
        .request(&OrchestratorRequest::ExecRemoteContainer {
            host_id: prod_host,
        ...
        .await
    {
        ...
        Ok(resp) => (
            ...
            Json(serde_json::json!({
                "success": false,
                "error": format!("Failed to query prod status: {}", resp.error.unwrap_or_default()),
            })),
        )
            .into_response(),
...
async fn get_prod_logs(
        ...
        .await
    {
        ...
        Ok(resp) => (
            ...
            Json(serde_json::json!({
                "success": false,
                "error": format!("Failed to query prod logs: {}", resp.error.unwrap_or_default()),
            })),
        )
            .into_response(),
        Err(e) => ipc_err_response(e),
...
async fn prod_exec(
    ...
    let command = match body.get("command").and_then(|v| v.as_str()) {
        ...
        None => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"success": false, "error": "command (string) required"})),
            )
                .into_response()
        }
    };
        ...
        .await
    {
        ...
        Ok(resp) => (
            ...
            Json(serde_json::json!({
                "success": false,
                "error": format!("Failed to execute command: {}", resp.error.unwrap_or_default()),
            })),
        )
            .into_response(),
        Err(e) => ipc_err_response(e),
...
async fn prod_push(
    ...
    let remote_path = match headers.get("X-Remote-Path").and_then(|v| v.to_str().ok()) {
        ...
        None => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"success": false, "error": "X-Remote-Path header required"})),
            )
                .into_response()
        }
    };
    ...
    if body.is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({"success": false, "error": "Empty body"})),
        )
            .into_response();
    }
    ...
    let archive_path = format!("/tmp/push-{}.tar", tmp_id);
    if let Err(e) = tokio::fs::write(&archive_path, &body).await {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({"success": false, "error": format!("Failed to write temp file: {e}")})),
        )
            .into_response();
    }
    ...
    match state
        .orchestrator
        .request_long(&OrchestratorRequest::ProdPush {
            dev_id: dev_id.clone(),
        ...
        .await
    {
        Ok(resp) if resp.ok => {
            ...
            info!(dev_id, remote_path, bytes = body_len, "Pushed to prod container via IPC");
            Json(serde_json::json!({
                "success": true,
