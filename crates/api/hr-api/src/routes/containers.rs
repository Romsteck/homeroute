...
async fn create_container(
    ...
    let task = state
        .task_store
        .create_task(
            TaskType::ContainerCreate,
            &format!("Création conteneur {}", container_name),
    ...
    let step = task_ctx.step("create", "Envoi de la requête de création").await;
        ...
        .await
    {
        Ok(resp) if resp.ok => {
            step.complete().await;
            // Spawn a tracker that watches agent_status for this container
            let events = state.events.clone();
            let cn = container_name.clone();
            let ctx = task_ctx.clone();
            tokio::spawn(async move {
                let step = ctx.step("wait_agent", "Attente connexion agent").await;
                let mut rx = events.agent_status.subscribe();
                ...
                loop {
                    tokio::select! {
                        result = rx.recv() => {
                            match result {
                                Ok(ev) if ev.slug == cn || ev.app_id == cn => {
        ...
        Ok(resp) => {
            ...
            step.fail(err_msg).await;
            task_ctx.fail(err_msg).await;
        ...
        Err(e) => {
            let err_msg = format!("Orchestrator unavailable: {}", e);
            step.fail(&err_msg).await;
            task_ctx.fail(&err_msg).await;
            ipc_err_response(e)
        }
...
async fn delete_container(
    ...
    let task = state
        ...
        .create_task(
            ...
            &format!("Suppression conteneur {}", id),
            TaskTrigger::User("admin".to_string()),
            Some(&id),
        )
        .await;
    ...
    let cleanup_step = task_ctx.step("cleanup_routes", "Nettoyage des routes proxy et DNS").await;
    ...
    cleanup_step.complete().await;

    let delete_step = task_ctx.step("delete", "Suppression du conteneur").await;
    match state
        .orchestrator
    ...
    {
        Ok(resp) if resp.ok => {
            delete_step.complete().await;
            task_ctx.done().await;
            Json(serde_json::json!({"success": true, "task_id": task_id})).into_response()
        }
        Ok(resp) => {
            let err = resp.error.as_deref().unwrap_or("Unknown error");
            delete_step.fail(err).await;
        ...
        Err(e) => {
            let err_msg = format!("Orchestrator unavailable: {}", e);
            delete_step.fail(&err_msg).await;
            task_ctx.fail(&err_msg).await;
            ipc_err_response(e)
        }
    }
