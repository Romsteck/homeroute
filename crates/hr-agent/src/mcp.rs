...
pub fn generate_settings_json(is_dev: bool) -> String {
    ...
    // Deploy tools (dev only)
    if is_dev {
        for t in get_deploy_tool_definitions() {
            if let Some(name) = t.get("name").and_then(|n| n.as_str()) {
                allow.push(format!("mcp__deploy__{name}"));
            }
        }
    }
...
WantedBy=multi-user.target"#;
                ...
                AppStack::LeptosRust => {
                    ...
                    // Step 1: cargo-leptos build --release
                    report.push("## Step 1: Building Leptos app (SSR + WASM)...".to_string());
                    let path_env = format!(
                        "/root/.cargo/bin:{}",
                        std::env::var("PATH").unwrap_or_default()
                    );
                    if dry_run {
                        report.push("DRY RUN: Would run `cargo-leptos build --release` in /root/workspace".to_string());
                    } else {
                        let build_output = tokio::process::Command::new("/root/.cargo/bin/cargo-leptos")
                            .args(["build", "--release"])
                            .current_dir("/root/workspace")
                            .env("PATH", &path_env)
                            .output()
                        ...
                        if !build_output.status.success() {
                            let stderr = String::from_utf8_lossy(&build_output.stderr);
                            report.push(format!("FAILED: cargo-leptos build --release\n{}", stderr));
                            return Ok(text_result(report.join("\n\n")));
                        }
                        report.push("OK: cargo-leptos build --release".to_string());
                    }
                    ...
                    // Step 3: Push target/site/ to /opt/app/site/ on PROD
                    let site_dir = std::path::Path::new("/root/workspace/target/site");
                    if site_dir.exists() {
                        report.push("## Step 3: Pushing site assets to PROD...".to_string());
                        if dry_run {
                            report.push("DRY RUN: Would push target/site/ to /opt/app/site/".to_string());
                        } else {
                            let tar_path = "/tmp/deploy-leptos-site.tar.gz";
                            let tar_output = tokio::process::Command::new("tar")
                                .args(["czf", tar_path, "-C", "/root/workspace/target/site", "."])
                                .output()
                                .await
                                .map_err(|e| format!("Failed to create site tarball: {e}"))?;
                            if tar_output.status.success() {
                                let archive_data = tokio::fs::read(tar_path)
                                    .await
                                    .map_err(|e| format!("Failed to read site tarball: {e}"))?;
                                let _ = tokio::fs::remove_file(tar_path).await;
                                let archive_size = archive_data.len();
                                ...
                                let push_resp = push_client
                                    .post(&push_url)
                                    .header("Content-Type", "application/octet-stream")
                                    .header("X-Remote-Path", "/opt/app/site")
                                    .header("X-Is-Directory", "true")
                                    .body(archive_data)
                                    .send()
                                    .await
                                    .map_err(|e| format!("Failed to push site assets: {e}"))?;
                                ...
                                if push_body
                                    ...
                                    .unwrap_or(false)
                                {
                                    report.push(format!("OK: Site assets pushed ({} bytes)", archive_size));
                                } else {
                                    let err = push_body
                                        .get("error")
                                        .and_then(|v| v.as_str())
                                        .unwrap_or("unknown");
                                    report.push(format!("WARNING: Site push failed: {}", err));
                                }
                            } else {
                                report.push("WARNING: Failed to create site tarball".to_string());
                            }
                        }
                    } else {
                        report.push("## Step 3: Site assets push SKIPPED (no target/site/)".to_string());
                    }

                    // Step 4: Deploy the binary from target/server/release/
                    report.push("## Step 4: Deploying binary...".to_string());
                    let mut binary_path = None;
                    if let Ok(mut entries) =
                        tokio::fs::read_dir("/root/workspace/target/server/release").await
                    {
                    ...
                    if dry_run {
                        match binary_path {
                            ...
                            None => {
                                report.push("DRY RUN: No binary found in target/server/release/ (would need cargo-leptos build first)".to_string());
                            }
                        }
                    } else {
                        let binary_path = match binary_path {
                            Some((p, _)) => p,
                            None => {
                                report.push("FAILED: No executable binary found in target/server/release/".to_string());
                                return Ok(text_result(report.join("\n\n")));
                            }
                        };
                    ...
                    // Step 5: Create/update app.service and restart
                    report.push("## Step 5: Deploying service...".to_string());
                    if dry_run {
                        report.push("DRY RUN: Would create/update app.service and restart".to_string());
                    } else {
                        let service_content = r#"[Unit]
Description=Application Service (Leptos)
After=network.target
...
WantedBy=multi-user.target"#;
        ...
        "dev_health_check" => {
            ...
            let services: Vec<&str> = match ctx.stack {
                ...
                AppStack::LeptosRust => vec!["code-server.service", "cargo-leptos-dev.service"],
                AppStack::ViteRust => vec!["code-server.service", "vite-dev.service", "cargo-dev.service"],
            };
            ...
            let ports: Vec<(u16, &str)> = match ctx.stack {
                ...
                AppStack::LeptosRust => vec![(13337, "code-server"), (3000, "cargo-leptos")],
                AppStack::ViteRust => vec![(13337, "code-server"), (5173, "vite-dev"), (3000, "app")],
            };
